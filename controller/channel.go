...
func FetchModels(c *gin.Context) {
	...
	// Keep this endpoint consistent with the existing-channel fetch endpoint:
	// task channels use their static adapter model list, while API channels use
	// their provider-specific model-list endpoint.
	...
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": fmt.Sprintf("获取模型失败: %s", err.Error()),
		})
		return
	}
